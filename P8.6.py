...
def reverse(root):
    if root is None: #루트 노드가 None이라면 None반환
        return None
    else: #루트 노드가 None이 아니면
        temp = root.left #임시 변수 temp를 사용하야
        root.left = root.right #루트의 왼쪽 및 오른쪽 하위 트리를 교체
        root.right = temp #왼쪽이나 오른쪽 하위 트리에서
        reverse(root.left) #재귀적으로 호출하여 좌우로 대칭 시킴
        reverse(root.right)
        return root #root 반환
